...
float pulseMin = -0.25;
float pulseMax = -0.25;
float pulseStrengthMin = 3;
float pulseStrengthMax = 7;
...
vec4 getheightFromCracks(vec4 _wave)
{

    float s11 = _wave.x;
    float s01 = textureOffset(cracksTexture, uvs, off.xy).x;
    float s21 = textureOffset(cracksTexture, uvs, off.zy).x;
    float s10 = textureOffset(cracksTexture, uvs, off.yx).x;
    float s12 = textureOffset(cracksTexture, uvs, off.yz).x;
...
void main()
{
	...
	vec4 heightFromCracks = getheightFromCracks(cracksColor);
	...
	//fragColor = vec4(1.0) * ((sin(time * pulsatingSpeed) * 0.5) + 0.5);
//	fragColor = ((ambient + diffuse + specular) * texture(normalTexture,uvs)) * sin(time);
//	fragColor = texture(normalTexture, texScale) * texture(cracksTexture, texScale);
